#2) Define a class  FamilyMember that has at least 5 properties (e.g., name, sex, 
#status, age, children) and then create 5 instances of that class, each of which 
#should be assigned to the following variables: fm1, fm2, fm3, fm4, fm5.  Define 
#two methods – parent? and child? that will test whether an instance of 
#this class is a parent or a child and what type of parent or child (father, mother, 
#son, daughter, dog). The method should make use of two or more properties in 
#doing this test.
...
#The purpose of this section is to prove that the child?/parent? methods work as intended.
#An array consisting of a Boolean and a String are returned from the methods, firstly returning 
#a Boolean for if they are a child/parent and secondly returning the type of child/parent as a String, where applicable.
family.each { |member| 
	          puts member 
			  p member.parent?
            }

puts " \n" #A new line, simply to enhance readability in the Command Line Interface.

#The same test as before, but for the child method.
family.each { |member| 
			   puts member
 			   p member.child?
 			}