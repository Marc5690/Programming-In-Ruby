...
module Util
   ...
   def self.fetch(string_item, out = [])
       all = Song.contents + Actor.contents + Album.contents
...
class Array
  def clean_up()
    #Removing the join and split methods retains the same functionality(Due to uniq) while requiring less memory
    self.uniq#this could be more elegant
  end
end
